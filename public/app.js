...
const I18N = {
  ja: {
    ...
    feature1:"✅ 会員登録・ログイン", feature2:"✅ 投稿・いいね・コメント", feature3:"✅ 学生証または教育機関メール認証", feature4:"✅ 大学別掲示板と共通掲示板",
    login:"ログイン", signup:"新規登録", email:"メールアドレス", password:"パスワード", password6:"パスワード 6文字以上", realName:"氏名", nickname:"ニックネーム", university:"大学名", faculty:"学部", fillAdmin:"管理者で入力", signupNote:"登録後、学生証画像をアップロードするか、教育機関メールで登録すると自動で認証されます。",
  ...
  ko: {
    ...
    feature1:"✅ 회원가입·로그인", feature2:"✅ 글·좋아요·댓글", feature3:"✅ 학생증 또는 교육용 이메일 인증", feature4:"✅ 대학별 게시판과 공통 게시판",
    login:"로그인", signup:"회원가입", email:"이메일", password:"비밀번호", password6:"비밀번호 6자 이상", realName:"실명", nickname:"닉네임", university:"대학명", faculty:"학부", fillAdmin:"관리자 계정 입력", signupNote:"가입 후 학생증을 업로드하거나 교육용 이메일로 가입하면 자동으로 인증됩니다.",
  ...
  en: {
    ...
    feature1:"✅ Signup and login", feature2:"✅ Posts, likes, and comments", feature3:"✅ Student ID or educational email verification", feature4:"✅ Separate boards by university and common boards",
    login:"Log in", signup:"Sign up", email:"Email", password:"Password", password6:"Password, 6+ chars", realName:"Real name", nickname:"Nickname", university:"University", faculty:"Faculty", fillAdmin:"Fill admin account", signupNote:"After signup, you can be verified automatically by uploading a student ID image or registering with an educational email.",
...
// Show terms of service. In a production app these would link to proper pages. Here we use simple alerts per language.
function showTerms() {
  if (lang === "ko") {
    alert("이용약관: 이 서비스는 일본 대학생을 위한 익명 커뮤니티입니다. 게시물 내용에 대한 책임은 작성자에게 있습니다.");
  } else if (lang === "en") {
    alert("Terms of Service: This service is an anonymous community for university students in Japan. Users are responsible for their own posts.");
  } else {
    alert("利用規約：このサービスは日本の大学生向けの匿名コミュニティです。投稿内容の責任は投稿者に帰属します。");
  }
}

// Show privacy policy. In a production app these would link to proper pages. Here we use simple alerts per language.
function showPrivacy() {
  if (lang === "ko") {
    alert("개인정보 처리방침: 입력된 정보는 인증과 통계 용도로만 사용되며, 서비스 외부로 공개되지 않습니다.");
  } else if (lang === "en") {
    alert("Privacy Policy: Submitted information is used only for verification and statistical purposes and is not shared externally.");
  } else {
    alert("プライバシーポリシー：入力された情報は認証と統計目的のみで使用され、外部には公開されません。");
  }
}
...
// Removed fillAdminUser to prevent exposing admin credentials
// function fillAdminUser(){} // intentionally left blank
...
async function toggleComments(id){
  ...
  // Build comments HTML with anonymous numbers and delete buttons if allowed
  const commentsHtml = data.comments.map(c => {
    return `<div class="comment">
      <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;">
        <b>${esc(c.nickname)}</b>
        ${c.canDelete ? `<button class="pill-btn" onclick="deleteComment('${id}','${c.id}')">${t("delete")}</button>` : ""}
      </div>
      <div>${esc(c.body)}</div>
    </div>`;
  }).join("");
...
async function deleteComment(postId, commentId){
  ...
  try {
    ...
    await renderPosts();
    // Wait briefly then reopen comments to reflect deletion
